...
local function fIsArray(i_tTable)
	...
	return table_maxn(i_tTable) == #i_tTable
end
...
local function fDecodeObject(i_sStr, i_nStartPos, i_bUnicode)
	...
	local nCurPos = i_nStartPos + 1	
	while true do
		if nCurPos > nStrLen then print("json decode object error 1", nCurPos, nStrLen) return end
		nCurPos = scan_whitespace(i_sStr, nCurPos)
		local sCurChar = string_sub(i_sStr, nCurPos, nCurPos)
		if sCurChar == '}' then break end
		if sCurChar ~= '"' then print("json decode object error 2", sCurChar) return end
		local key, value
		key, nCurPos = fDecodeString(i_sStr, nCurPos, i_bUnicode)
		if nCurPos == nil then print("json decode object error 3") return end
		nCurPos = scan_whitespace(i_sStr, nCurPos)
		if nCurPos > nStrLen then print("json decode object error 4", nCurPos, nStrLen) return end
		sCurChar = string_sub(i_sStr, nCurPos, nCurPos)
		if sCurChar ~= ":" then print("json decode object error 5", sCurChar) return end
		nCurPos = nCurPos + 1
		nCurPos = scan_whitespace(i_sStr, nCurPos)
		if nCurPos > nStrLen then print("json decode object error 6", nCurPos, nStrLen) return end
		value, nCurPos = fDecode(i_sStr, nCurPos, i_bUnicode)
		if nCurPos == nil then print("json decode object error 7") return end
		tTemp[key] = value
		nCurPos = scan_whitespace(i_sStr, nCurPos)
		if nCurPos > nStrLen then print("json decode object error 8", nCurPos, nStrLen) return end
		sCurChar = string_sub(i_sStr, nCurPos, nCurPos)
		if sCurChar == ',' then nCurPos = nCurPos + 1 end
	end
	return tTemp, nCurPos + 1
