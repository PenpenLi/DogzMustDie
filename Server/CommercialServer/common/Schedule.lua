...
-- 定时器周期类型
local ScheduleTaskCycleTypeEnum = {
    eSecond = 1, -- 周期单位秒
	eMinute = 2, -- 周期单位分钟
	eHour	= 3, -- 周期单位小时
	eDay	= 4, -- 周期单位天
...
RegistEnum("ScheduleTaskCycleTypeEnum", ScheduleTaskCycleTypeEnum);
...
-- CScheduleTask
--
local CScheduleTask = RegistClass("CScheduleTask")
...
function CScheduleTask:Action()
	local res1, res2 = pcall(self.m_fFunc, unpack(self.m_tParam))
	if not res1 then
		print("ERROR!!!", res2)
	end
...
local function print_collect_memory()
...
function CSchedule:Initialize()
	...
	self.m_nGlobalID = 0
	-- collectgarbage
	self:AddTask({}, ScheduleTaskCycleTypeEnum.eMinute, 10, 0, print_collect_memory, {})
	-- dayrefresh
	self:AddTask({m_sTime = "00:00"}, ScheduleTaskCycleTypeEnum.eDay, 1, 0, day_refresh_singleton, {})
	return true
end
...
function CSchedule:AddTask(i_tTime, i_nCycleType, i_nCycleTime, i_nRunTime, i_fFunc, i_tParam, i_bIsTest)
	...
	local task = NewClass("CScheduleTask", self.m_nGlobalID, actionTime, cycleTime, i_nRunTime, i_fFunc, i_tParam)
	self:AddTaskInList(task)
	return self.m_nGlobalID
end
