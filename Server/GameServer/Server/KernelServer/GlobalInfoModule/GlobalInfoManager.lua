...
function CGlobalInfoManager:Initialize()
    ...
    local dbversion = res[1].dbversion
...
function CGlobalInfoManager:CalServerData()
    -- if ServerInfo.isbridge then return end
    -- local res = CDBService:Execute('select sum(gold) as gold, sum(bindgold) as bindgold from item_system', self.m_nServerID)
    -- if res and res[1] then
    --     res = res[1]
    --     -- CDataLog:GspLogServerData(self.m_nServerID, 1, res.money)       -- 银两
    --     CDataLog:GspLogServerData(self.m_nServerID, 2, res.gold)        -- 元宝
    --     CDataLog:GspLogServerData(self.m_nServerID, 5, res.bindgold)    -- 绑元
    -- end
    -- res = CDBService:Execute("select count(*) as ct from marry", self.m_nServerID)
    -- if res and res[1] then
    --     CDataLog:GspLogServerData(self.m_nServerID, 10, res[1].ct)         -- 结婚对数
    -- end
    -- res = CDBService:Execute([[select sum(honour) as honour, sum(prestige) as prestige, 
    --                         sum(guildcontr) as guildcontr, sum(marryvalue) as marryvalue,
    --                         sum(realgas) as realgas, sum(mijivalue) as mijivalue from role_currency]], self.m_nServerID)
    -- if res and res[1] then
    --     res = res[1]
    --     CDataLog:GspLogServerData(self.m_nServerID, 3, res.honour)              -- 荣誉
    --     CDataLog:GspLogServerData(self.m_nServerID, 4, res.prestige)            -- 功勋
    --     CDataLog:GspLogServerData(self.m_nServerID, 6, res.guildcontr)          -- 帮贡
    --     CDataLog:GspLogServerData(self.m_nServerID, 7, res.marryvalue)          -- 情缘点
    --     CDataLog:GspLogServerData(self.m_nServerID, 8, res.realgas)             -- 真气
    --     CDataLog:GspLogServerData(self.m_nServerID, 9, res.mijivalue)           -- 监印文本
    -- end
end
...
function CGlobalInfoManager:Save(i_bShut)
	...
	if i_bShut then
		...
		if ServerInfo.isbridge then
			local CBridgeListener  = RequireSingleton("CBridgeListener");
