...
local function onRecv(session, data, len)
	...
	if not ProtectedCall(function() OnFancyCall(oPlayer, fancy_decode(data, len)) end) then
...
local CClientListener = RequireSingleton("CClientListener");
function CClientListener:Initialize()
    local ip = "0.0.0.0";
    print("CClientListener start listen at", ip .. ":" .. ServerInfo.clientport)
	self.m_oFancyListener = _netservice.listenfancyWebsocket(
        ip, ServerInfo.clientport,
