...
--local
local CPlayerSystemManager = RequireClass("CPlayerSystemManager");
...
function CPlayerSystemManager:OnMarry()
    local res = true;
    for _, v in ipairs(self.m_tSystemSet) do
        if not ProtectedCall(function() v:OnMarry() end) then
            res = false;
        end
	end
    return res;
