...
InitializeSingleton = function()
	...
	for k, v in ipairs(tSingletonSet) do
		if v.Initialize then
            ...
            --print("InitializeSingleton", v._name, res1, res2);
			if (not res1) or (not res2) then
                print("ERROR!!! InitializeSingleton Failed", v._name, res1, res2);
                return false;
			end
		end
	end
...
WeekRefreshSingleton = function()
	for k, v in ipairs(tSingletonSet) do
		local OnWeekRefresh = v.OnWeekRefresh;
		if v.OnWeekRefresh then
			-- print("WeekRefreshSingleton", v._name);
