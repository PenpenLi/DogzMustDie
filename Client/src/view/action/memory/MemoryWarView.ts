...
module H52D_Framework {
	...
	export class MemoryWarView extends ui.action.memory.MemoryWarViewUI {
		/**选择的上阵位置 */
		...
		constructor(buf) {
			...
			MemoryLogic.Instance.InitPosInfo();
			this.on(Laya.Event.REMOVED, this, this.OnDestroy);
			Event.RegistEvent('OnMouseUp', Laya.Handler.create(this, this.MouseUp));

			if (GetTabLength(MemoryLogic.Instance.war) == 0) {
				MemoryLogic.Instance.war = MemoryLogic.Instance.war;
			}
			// 缓存已有英雄列表
		...
		private DragHero(nHeroID, bShowDrag) {
			...
			if (!MemoryLogic.Instance.IsInWar(nHeroID)) {
		...
		private ListHeadDown(nHeroID, item, event) {
			Tick.Once(300, this, this.OneListHeadDown, [nHeroID, this.memory_listwar._childs[1].value, item, event])
		}
		...
		private OneListHeadDown(nHeroID, scrollBarY, item, event) {
			...
			bool = MemoryLogic.Instance.IsInWar(nHeroID);
			this.DragHero(nHeroID, bool);
		}

		/** 按住阵容上的英雄 */
		private CurMouseDown(pos) {
			let tPositionWar = MemoryLogic.Instance.war
		...
		private MouseUp(): void {
			...
			let bool = MemoryLogic.Instance.IsInSaveWar(nNowPos, nDragHeroID)
			let hero_id = MemoryLogic.Instance.war[this._opos];
			if (bool) {
				if (hero_id) {
		...
		private PutHero(nNowPos: number, nDragHeroID: number) {
			// 放置英雄
			MemoryLogic.Instance.PutHero(nNowPos, nDragHeroID)
			MemoryLogic.Instance.bChange = true
			this.UpdatePosView()
			this.UpateHeroLsit()
		...
		private SetHandler(item, index: number): void {
			...
			let lv = hero.Level;
			duihao.skin = "";
			let herolist = MemoryLogic.Instance.HeroWar
			let a = MemoryLogic.Instance.IsInWar(nHeroID);
			if (a) {
			...
			let bool = MemoryLogic.Instance.IsInWar(nHeroID);
			gowar.visible = bool;
		...
		/** 点击保存按钮 */
		public SaveBtnClick() {
			let heroposInfo = MemoryLogic.Instance.war;
			let bool = MemoryLogic.Instance.bChange;
			...
			heroposInfo = MemoryLogic.Instance.getwar();
			...
			MemoryLogic.Instance.K_SaveCopyBattleArray(this._eTpye, heroposInfo);
			MemoryLogic.Instance.bChange = false;
		}
		...
		private Btn_Close() {
			...
			if (MemoryLogic.Instance.bChange) {
				let str = SysPromptConfig[10012].strPromptInfo;
				TipsLogic.Instance.OpenMessageBox(str,
					Laya.Handler.create(this, () => {
						this.SaveBtnClick();
						UIManager.Instance.DestroyUI("MemoryWarView", [ViewToppestRoot]);
					}),
					Laya.Handler.create(this, () => {
						UIManager.Instance.DestroyUI("MemoryWarView", [ViewToppestRoot]);
					})
