...
module H52D_Framework {
	...
	export class KickingPetView extends ui.action.kicking.KickingPetViewUI {
		...
		private SetTopPetHit() {
			...
			let petId = KickingLogic.Instance.petId;
		...
		private SetCurrentPetInfo() {
			...
			let currentId = KickingLogic.Instance.petId;
			if (currentId == 0) {
				return;
			}
		...
		private Pethandle(item, index) {
			...
			choice.visible = KickingLogic.Instance.petId == pet.ID;
		...
		private ChoiceCurrentPet(id: number) {
			...
			if (KickingLogic.Instance.petId != id) {
				this.SetCurrentPetInfo();
				//这是PVP上阵
				KickingLogic.Instance.petId = id;
				if (GetTabLength(KickingLogic.Instance.fireData.Pet) == 0) {
					KickingLogic.Instance.fireData.Pet[0] = { id: 1, Level: 1 }
				}
				KickingLogic.Instance.fireData.Pet[0].id = id;
				KickingLogic.Instance.fireData.Pet[0].Level = PetManager.Instance.OwnPetList[id].Level;
			}
			this.UpdateList();
