...
module H52D_Framework {
	AddViewResource("KickingWarView",
		[
	...
	export class KickingWarView extends ui.action.kicking.KickingWarUI {
		...
		constructor(buf) {
			...
			if (GetTabLength(KickingLogic.Instance.war) == 0) {
				KickingLogic.Instance.war = HeroPosition.Instance.PositionWar;
			}
			// 缓存已有英雄列表
		...
		private DragHero(nHeroID, bShowDrag) {
			...
			if (!KickingLogic.Instance.IsInWar(nHeroID)) {
		...
		private OneListHeadDown(nHeroID, scrollBarY, item, event) {
			...
			bool = KickingLogic.Instance.IsInWar(nHeroID);
			this.DragHero(nHeroID, bool);
		}

		/** 按住阵容上的英雄 */
		private CurMouseDown(pos) {
			let tPositionWar = KickingLogic.Instance.war
		...
		private MouseUp(): void {
			...
			let bool = KickingLogic.Instance.IsInSaveWar(nNowPos, nDragHeroID)
			let hero_id = KickingLogic.Instance.war[this._opos];
			if (bool) {
				if (hero_id) {
		...
		private PutHero(nNowPos: number, nDragHeroID: number) {
			// 放置英雄
			KickingLogic.Instance.PutHero(nNowPos, nDragHeroID)
			KickingLogic.Instance.bChange = true
			this.UpdatePosView()
			this.UpateHeroLsit()
		...
		private SetHandler(item, index: number): void {
			...
			let lv = hero.Level;
			duihao.skin = "";
			let herolist = KickingLogic.Instance.HeroWar
			let a = KickingLogic.Instance.IsInWar(nHeroID);
			if (a) {
			...
			let bool = KickingLogic.Instance.IsInWar(nHeroID);
			gowar.visible = bool;
		...
		/** 点击保存按钮 */
		public SaveBtnClick() {
			let heroposInfo = KickingLogic.Instance.war;
			let bool = KickingLogic.Instance.bChange;
			...
			heroposInfo = KickingLogic.Instance.getwar();
			...
			KickingLogic.Instance.KickingWarInfo(heroposInfo);
			KickingLogic.Instance.bChange = false;
		}
		...
		private Btn_Close() {
			...
			if (KickingLogic.Instance.bChange) {
