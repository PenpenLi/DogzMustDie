...
module H52D_Framework {
	...
	export class SkillObject {
		...
		private PlaySkillEffect() {
			...
			let time: number = this._skillData.flyEffect["7"];
		...
		private LoadHitEffect(x: number, y: number) {
			if (this._hitAvatar) {
				...
				this._hitAvatar = new Avatar(AvatarEffectRoot)
